#!/usr/bin/env python3
"""
ImageNet inference — classify images using a trained ViT checkpoint.

Usage:
    python vit_imagenet_classifier.py --model ../models/vit_large_imagenet_best.pth --image photo.jpg
    python vit_imagenet_classifier.py --model ../models/vit_large_imagenet_best.pth --test
"""
import argparse
import io
import glob
...
VIT_MODELS = {
    ...
    "large": torchvision.models.vit_l_16,
    "huge":  torchvision.models.vit_h_14,
}
...
def get_class_names():
    ...
    _CLASS_NAMES = ViT_L_16_Weights.IMAGENET1K_V1.meta["categories"]
    return _CLASS_NAMES


def load_model(path, device):
    ckpt = torch.load(path, map_location=device, weights_only=True)
    model_size = ckpt.get("model_size", "large")
    model_fn = VIT_MODELS[model_size]
    model = model_fn(weights=None, num_classes=1000)
    ...
    print(f"Loaded: {path}")
    print(f"  ViT-{model_size}  |  Epoch: {epoch}  |  Top-1: {top1}%  |  Top-5: {top5}%")
    return model
