#!/usr/bin/env python3
"""
Vision Transformer (ViT) distributed training on ImageNet-1K.
...
def parse_args():
    ...
    parser.add_argument("--model-size",  choices=list(VIT_MODELS.keys()), default="large",
...
# ---------------------------------------------------------------------------
# Image transforms — ViT uses 224x224 (vit_b_16, vit_l_16) or 224x224 (vit_h_14)
# ---------------------------------------------------------------------------
...
def save_checkpoint(model, optimizer, epoch, top1, top5, config,
    ...
    size = config.get("model_size", "large")
    suffix = "best" if is_best else "latest"
    path = os.path.join(model_dir, f"vit_{size}_imagenet_{suffix}.pth")
...
def train_func(config):
    ...
    model_info = VIT_MODELS[config["model_size"]]
    model = model_info["fn"](weights=None, num_classes=1000)
    ...
    if world_rank == 0:
        ...
        print(f"ViT-{config['model_size']} parameters: {n_params/1e6:.1f}M")
        effective_batch = config["batch_size"] * world_size
        scaled_lr = config["lr"] * effective_batch / 1024
        print(f"Per-GPU batch: {config['batch_size']} | "
              f"Effective batch: {effective_batch} | Scaled LR: {scaled_lr:.6f}")
    ...
    # 4. Optimizer — AdamW (standard for ViT, unlike SGD for ResNet)
    effective_batch = config["batch_size"] * world_size
    scaled_lr = config["lr"] * effective_batch / 1024
...
def main():
    ...
    nccl_ib = os.environ.get("NCCL_IB_DISABLE", "1")
    print(f"Model: ViT-{args.model_size} ({model_info['params']} params)")
    print(f"NCCL backend: {'InfiniBand' if nccl_ib == '0' else 'Ethernet'}")

    # Load ImageNet as Ray Datasets from parquet
    ...
    run_config = RunConfig(
        name=f"vit_{args.model_size}_imagenet",
    ...
    print(f"Starting ViT-{args.model_size} ImageNet: "
